...
function add_vm9() {
  ip netns add vm9
  ovs-vsctl add-port br-int vm9 -- set interface vm9 type=internal
  ip link set vm9 netns vm9
  ip netns exec vm9 ip link set vm9 address 02:ac:10:ff:00:02
  ip netns exec vm9 ip addr add 10.0.0.2/24 dev vm9
  ip netns exec vm9 ip link set vm9 up
  ip netns exec vm9 ip route add default via 10.0.0.1
  ip netns exec vm9 ip route add 192.168.10.0/24 via 10.0.0.201
  ip netns exec vm9 ip route add 192.168.20.0/24 via 10.0.0.201
  ovs-vsctl set Interface vm9 external_ids:iface-id=inside-vm9
}

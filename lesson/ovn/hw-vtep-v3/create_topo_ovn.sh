...
function add_vm10() {
  ...
  ip netns exec vm10 ip link set vm10 address 02:ac:10:ff:00:03
  ip netns exec vm10 ip addr add 10.0.0.3/24 dev vm10
  ip netns exec vm10 ip link set vm10 up
  ip netns exec vm10 ip route add default via 10.0.0.1
  ip netns exec vm10 ip route add 192.168.10.0/24 via 10.0.0.201
  ip netns exec vm10 ip route add 192.168.20.0/24 via 10.0.0.201
  ovs-vsctl set Interface vm10 external_ids:iface-id=inside-vm10
}
...
#ovn-nbctl create Logical_Router name=router_bm options:chassis=ovn
ovn-nbctl lrp-add router_bm external-port3 f0:00:00:00:00:03 192.168.209.77/19
ovn-nbctl lsp-add external router3-external-port \
          -- lsp-set-options router3-external-port router-port=external-port3 \
          -- lsp-set-type router3-external-port router \
          -- lsp-set-addresses router3-external-port router
...
ovn-nbctl lsp-add inside_bm inside-vm9
ovn-nbctl lsp-set-addresses inside-vm9 "02:ac:10:ff:00:02 10.0.0.2/24"
ovn-nbctl lsp-set-port-security inside-vm9 "02:ac:10:ff:00:02 10.0.0.2/24"

ovn-nbctl lsp-add inside_bm inside-vm10
ovn-nbctl lsp-set-addresses inside-vm10 "02:ac:10:ff:00:03 10.0.0.3/24"
ovn-nbctl lsp-set-port-security inside-vm10 "02:ac:10:ff:00:03 10.0.0.3/24"

ovn-nbctl lsp-add inside_bm inside-vm11
ovn-nbctl lsp-set-addresses inside-vm11 "02:ac:10:ff:00:04 10.0.0.4/24"
ovn-nbctl lsp-set-port-security inside-vm11 "02:ac:10:ff:00:04 10.0.0.4/24"
...
ovn-nbctl lr-nat-add router_bm snat 192.168.209.77 10.0.0.0/24
ovn-nbctl lr-nat-add router_bm snat 192.168.209.77 192.168.10.0/24
ovn-nbctl lr-nat-add router_bm snat 192.168.209.77 192.168.20.0/24
ovn-nbctl lr-route-add router_bm "0.0.0.0/0" 192.168.209.77
